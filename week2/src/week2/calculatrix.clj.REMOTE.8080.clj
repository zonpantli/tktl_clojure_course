...
(defn prepare-operand [command arg last-result var-table argnum]
  (cond
   (= arg "_")
   last-result 
   (and (get var-table arg) (= command "store") (= argnum 1))
   arg
   (get var-table arg)
   (get var-table arg)
...
(defn compute [command args & state]
  ...
  (let [last-result (first state)
        ...
        first-operand (prepare-operand command (first args) last-result var-table 1)
        second-operand (prepare-operand command (second args) last-result var-table 2)]
    (cond
